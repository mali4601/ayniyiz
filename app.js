...
function renderLoggedIn(user,credits=0){
  ...
  if(box){
    box.innerHTML=`
      ${creditHtml(credits)}
      <div class="userEmail">${user.email || "Kullanıcı"}</div>
...
async function ensureUserDoc(user){
  ...
  if(Object.keys(patch).length>1) await updateDoc(refDoc,patch);
...
window.registerUser=async()=>{
  ...
  try{
    ...
    await afterLogin(u.user,"Kayıt başarılı ⚡ 1 kredi hesabına tanımlandı.");
